...
from models import nin_wiki, nin, resnet
    ...
    if args.model_type == 0:
        ...
        if args.bn_fuse:
            ...
            if args.prune_quant or args.prune_qaft:
                ...
                torch.save(state, "models_save/mnist_qat4.pth")
    else:
        if args.bn_fuse:
...
def test():
    ...
    for data, target in testloader:
        ...
        for i in range(target.data.__len__()):
            label = target.data[i]
            class_correct[label] += correct_numpy[i].item()
            class_total[label] += 1

    '''for i in range(class_correct.__len__()):
        print("class_correct",class_correct[i])
...
if __name__ == "__main__":
    ...
    #transform_train.transforms.insert(0, transforms.Grayscale(num_output_channels=1))
    ...
    #transform_test.transforms.insert(0, transforms.Grayscale(num_output_channels=1))
    ...
    if args.prune_quant:
        ...
        if args.model_type == 0:
            ...
            model = nin_wiki.Net(cfg=checkpoint["cfg"])
